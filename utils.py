...
def roleEmoji(name:str):
    try:
        ...
        try:
            return classes.role.Role.toRole(name).emoji
        except:
            return ""

def isContraction(name:str):
    for val in classes.contraction.Contraction.allContractions:
        if name.lower() == val.display_name.lower():
            return True
    else:
        return False
